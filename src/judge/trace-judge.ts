 ...
 * tracevals-specific — the per-plan budget gate, the trace-worded verdict
...
/**
 * tracevals' own verdict wording. Structurally identical to the library's
...
export function makeTraceJudge(options: TraceJudgeOptions): TraceJudge {
  ...
  const cache = new JsonCache<JudgeRun[]>(
    options.cacheDir ?? ".tracevals/cache",
