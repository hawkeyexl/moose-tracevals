...
**Scope:** the five target segments for the tracevals documentation set and the axis that
...
tracevals exists because instructions and behavior have drifted apart. Someone writes a `SKILL.md`,
...
| [`aud-toolsmiths`](toolsmiths.md) | Toolsmiths — **cross-cutting** | Code built on top of tracevals | Narrow: the programmatic API and the extension seams. |
...
ownership axis — it is defined by *what someone builds on top of tracevals* rather than by which
