...
docs_owner: they own the criteria standard, not the instruction artifacts
...
violation carries, how the criteria schema evolves, and whether the judge can be trusted. It does
...
agent framework and set the criteria conventions their downstream users inherit.
...
Define a criteria standard that is honest — one where a passing result means something and a
...
2. **Calibrate and version.** Tune the judge until its verdicts are trustworthy, and change the
   standard over time without a silent reinterpretation of existing criteria.
...
- **Criteria that are permanently green.** A criterion that no session could fail inflates the pass
...
- **Schema evolution.** Changing the criteria block's shape breaks every artifact already declaring
  criteria unless versions coexist.

## Buying constraints

- The consensus and zone rules must be documented arithmetic, not adjectives.
- The criteria schema must be a published, versioned artifact they can pin.
- Grader options must be validated up front, so a malformed criterion is an error rather than a
  silent pass.
...
- **Subject dependencies:** *criterion*, *grader*, and *finding* must be established first — this
  ...
  *consensus* → *zone* → *outcome* must be introduced in that order; a zone threshold is
  meaningless before consensus is defined. `capability` vs `regression` depends on the criteria
