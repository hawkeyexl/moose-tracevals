...
Engineers and technical writers who have accumulated agent instructions in a repository: a handful
of skills, a few subagent definitions, and a project rules file that has grown past the point anyone
rereads it. They range from a solo maintainer with three skills to a platform team curating a shared
library of fifty, but the shape of the problem does not change with scale.
...
- **No feedback loop.** They wrote "always reproduce the bug with a failing test first" and have no
  ...
  sessions.
- **Silent drift.** An instruction that worked when written stops being followed after a model
...
- Must work against sessions that **already happened** — no instrumentation, no re-running work.
- Must not require an API key to try. The local agent CLI's own auth, or a mock provider, has to
  carry the first run.
- Must be reviewable: everything written lands in a diff they approve.
