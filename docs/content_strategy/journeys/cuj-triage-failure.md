...
steps:
  ...
  - { stage: "Tell an error from a failure", doc: "/moose-tracevals/triage/#error-means-the-check-broke", exists: true, note: "including a malformed evals block, which carries a line number" }
  - { stage: "Decide: fix the session, fix the eval, or escalate", doc: "/moose-tracevals/triage/#what-to-do-next", exists: true }
