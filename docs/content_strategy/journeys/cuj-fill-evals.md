...
id: cuj-fill-evals
type: cuj
title: Propose evals across a whole project and review the diff
...
# CUJ: Propose evals across a whole project and review the diff

**Scope:** bulk authoring with `fill`, the product's one write path. Authoring a single eval by
hand is [`cuj-declare-evals`](cuj-declare-evals.md); this journey assumes that vocabulary and
is about volume and trust.

**Trigger.** The reader has declared one eval, likes the result, and has counted the artifacts
in their repository. Hand-writing evals for all of them is the step that ends adoption.
...
- **Everything lands in a reviewable diff.** Existing evals are never modified; a name collision
  is an error rather than an overwrite; only the frontmatter block is rewritten.
- **Project rules are proposed but never written.** Evals inside a file the agent reads *before*
