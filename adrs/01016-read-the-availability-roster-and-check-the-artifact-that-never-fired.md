...
Artifact resolution starts from what the trace *used* — `trace.skillInvocations` and
`trace.agentSpawns` — so a skill that should have been invoked and was not resolves to nothing, is
...
shape — and corrected three assumptions that would each have shipped a wrong answer.

| `attachment.type` | Records | Carries |
|---|---|---|
| `skill_listing` | 606, in 280 of 281 files | `names`, `skillCount`, `isInitial`, and `content` — one `- name: description` line per skill, including scope notes like *"from src/common/.claude/skills — applies when working on files under src/common/"* |
...
2. **A description is genuinely optional.** `content` is capped near 30,000 characters. Past the
   ...
   "has none" — and 590 lines across the corpus are continuation lines of a description that wrapped,
   which a naive line-per-skill parse would drop or misattribute.
...
**Sidecar transcripts carry their own roster too** — 401 of 401 checked do — and it is a *different,
usually smaller* set than the parent's. That is a hazard, not a bonus: replaying a sidecar's listing
as the session's would withdraw everything the parent had, since a sidecar listing is `isInitial`
for that branch.

What the roster still does not give: artifact *bodies* or their hashes at session time, and the git
SHA the session ran against. That residual is a hook's job and is deliberately not in this change.

## Decision Drivers

- The roster works **retroactively, on every session already on disk**, and travels inside the trace
  — no filesystem scan, no `MOOSE_TRACEVALS_HOME` pinning, no machine that has to match.
- `schemas/artifact-evals-1.0.0-proposal.1.json` is docmeta's, vendored byte-identical (ADR 01010),
  ...
  deliberately open — "validated by the grader at run time — a grader's options evolve on the
  grader's schedule" — so anything expressed there is ours and needs no upstream proposal.
...
- ADR 01003's rule holds: a trace with no listing records is the ordinary case for older sessions
  and for stream transcripts, and must degrade to *unknown* rather than to a confident zero.
...
Chosen option: **parse the roster out of the transcript; express the condition in grader `options`;
skip an unmatched trigger; fold a branch's roster in name-wise; and carry all three coverage states
in one table with the unused ones behind a flag.**
...
`withdrawnAt`, both ordinals in `trace.events`. A name that is withdrawn and later re-offered becomes
two entries rather than one entry with a hole, so an interval is always answerable by comparison
rather than by replay — `availableAt(entries, index)` is a filter. Per-index history is kept because
the question a later phase asks is "available when this window opened", not "available at some
point". MCP servers carry a `status` (`pending`, `needs-auth`, `failed`); tools carry no description,
because `addedLines` there repeats the tool's own name.
...
Ordinals are remapped through ADR 01014's splice, because that renumbers `index` over the merged
list and a parse-time ordinal would otherwise point at the wrong event.
...
|---|---|---|
| not matched | — | `skipped`, reason names the unmet condition |
| matched | no | `fail` |
| matched | yes | `pass` |
...
green from an eval that did not run, and — worse — would do it on exactly the sessions the eval was
written to stay quiet about, so the report would look like coverage in proportion to how little it
checked.
...
proposal (ADR 01004), and **an unrecognised condition is an error rather than a silent drop** — a
...
over already-normalized paths, and it matches at a path-segment boundary the way `file-access`'s
suffix compare already does, so `docs/**` means the same thing wherever the checkout lives.
...
and never withdrawn. That keeps the main chain's intervals exact while making "was this ever on the
menu?" — the only question coverage asks — answer correctly for a skill offered to a subagent alone.
...
**configuration** bug, not an adherence failure, and a reader who cannot tell it from "offered and
ignored" goes looking in the wrong file. `offered-not-used` is summarised by default because 274 rows
would bury the evals; the flag flows through the resolved config as
...
but it is also the eval most likely to be noisy and it costs inference proportional to roster size.
...
- Good, because the negative space is checkable for the first time, with no new field, no upstream
  proposal, and no second inventory for anyone to maintain.
...
- Good, because "skill X was available, described as *Y*, and was never invoked" is now a **trace
  fact** rather than an inference, which is what makes it safe to report.
...
- Bad, because the roster is only as good as what Claude Code recorded. Its listing text is
  budget-truncated, so on a large roster most descriptions are simply absent, and the report says
...
- Bad, because `trace.availability` is required, so every synthetic `Trace` literal in the suite grew
  a field — the third time an ADR in this stack has widened the model (01013, 01014, and now this).
- Bad, because a conditional eval hung on `CLAUDE.md` is graded against the whole session, so its
...
- Bad, because a **built-in slash command reads as a not-offered skill.** The adapter turns every
  ...
  skills and the roster correctly does not list them — and the row already carries "not found (5
...
`test/unit/trace-availability.test.ts` pins the parse against each real record shape: the
`plugin:skill` join, a wrapped description, an absent one, delta-versus-initial replay, an agent
removal, a tool removed and readded producing two intervals, MCP status, a malformed record parsing
to nothing rather than throwing, and — the invariant that matters most — a trace with no listing
records reporting `recorded: false` rather than zero.

`test/unit/graders/when.test.ts` pins all three trigger states explicitly, including that an unmatched
trigger returns `skipped` and neither `pass` nor `fail`; that the empty-window reason outranks the
trigger reason; each predicate in both directions; conjunction; and every validation rejection,
including the unknown-condition one. `globToRegExp` is pinned against segment-boundary matching,
`docsite/` not matching `docs/**`, and regex metacharacters staying literal.
...
says "not found", that the summary reads "unknown" rather than "0 offered", and that the markdown
...
`agent_listing_delta`, and `deferred_tools_delta` before the first prompt, plus a mid-session
...
workflow asserts the armed trigger passing, the unarmed one skipping with a trigger-not-met reason,
`fix-bug` as `offered-and-used`, `doc-writer` as `not-offered`, the counts, that unused artifacts are
summarised by default, and — in its own step — that `--report-unused-artifacts` lists exactly the
four expected refs with their descriptions and without claiming a filesystem search, while leaving
the exit code alone. `claude-session-sidecar.jsonl` carries a main-chain listing after the splice
...
- Bad, because it is a second inventory to maintain, it goes stale silently, and the interesting case
  — a skill nobody remembered to expect — is exactly the one it cannot catch.

### `when` inside grader `options`

- Good, because `options` is open by schema decree, so it costs no schema change and no upstream
  proposal — and ADR 01010 just stopped this repo forking the vocabulary.
...
- Bad, because `inlineEval` is `additionalProperties: false` in a byte-identical vendored copy, so it
  cannot ship here at all until docmeta accepts it — and this repo would be blocked on another
...
- Bad, because it is the hollow-feature failure: green in exact proportion to how rarely the eval
  armed, and indistinguishable in the report from a check that ran and succeeded.
...
- Bad, because a sidecar's listing is `isInitial` for its own branch, so replaying it would withdraw
  the parent's entire roster mid-session and re-offer a subset — a fabricated churn that would make
...
- Bad, because a skill invoked inside a subagent but offered only there would be reported
  `not-offered` — a confident accusation of a configuration bug that does not exist.
...
- Bad, because two booleans' worth of information does not fit in one, and whichever pair it collapsed
