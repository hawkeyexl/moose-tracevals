...
union member was aspiration written into a type, and a reader of the JSON output would reasonably
...
no part of the pipeline noticing — and unlike the session axis, this bias is about the *yardstick*
...
Chosen option: **set it from `eval-provenance`**. `extractEvals` returns
`proposedBy: Map<string, string[]>` — eval id to the models that proposed it — `EvalPlan` carries
the entry for its own id, and the judge sets `axis: "criterion"` when the effective judge model
appears in that list.

A list rather than a string, because a re-fill by a second model *extends* the provenance block
rather than replacing it, so one id legitimately has two authors and either one judging it is the
same bias.
...
axis, have a *human* confirm the assertion — another model would still grade the same wording, so
switching models does not address it. Reporting the weaker remedy while the stronger one applies
...
`test/unit/self-preference.test.ts` pins the flag when the judge proposed the assertion, its
absence when another model did, its absence when nothing recorded an author, and the session
axis winning when both apply. `test/unit/evals.test.ts` pins the round trip out of frontmatter,
