...
one of a family of tools — docevals and docmeta are the others — that are routinely used together in
the same repository, and each brought its own dotfile. A project wiring up all three accumulated
three config files that nobody could see the relationship between, and the settings they duplicate
(provider selection, model, API key variable, cost ceilings) had to be restated and kept in sync in
each one.

Where should a tool in the family look for its configuration, so that one file can serve all of them
...
- The two ways an author can lose a whole config to this change — leaving keys un-nested, or leaving
  the old filename in place — must be reported, not silently defaulted through.
...
Chosen option: **a single `moose.config.yaml`, each tool reading its own top-level key** — this tool
reads `tracevals:`.
...
its `additionalProperties: false` still applies at every level below `tracevals:` and `parseConfig()`
keeps its existing signature and contract — it takes the section object, not the file.
...
- A file with no `tracevals:` key but with keys this tool owns at the top level (`judge:`,
  `provider:`, …) — the un-nested config. The stray keys are named in the error, and the list is
  derived from the schema's own `properties` so it cannot drift from the real key set.
...
- A directory holding the pre-centralization `moose-tracevals.config.yaml` and no
  ...
  key.
- A `moose.config.yaml` that exists but cannot be read — a directory by that name, a permission
  error. Only `ENOENT`/`ENOTDIR` counts as absent; anything else is reported rather than defaulted
  through, and does not reach the legacy check, which would otherwise claim the file is missing.
...
- Good, because the strictness that catches typos is unchanged for our own keys, while sibling keys
  cost nothing — no registry of known tools, no coordination, no version coupling between the tools.
...
- Bad, because every existing config gains a level of indentation, and every documented YAML example
  had to be re-nested.
- Bad, because the file root is unvalidated by construction, so a top-level key that is a genuine
  misspelling of `tracevals` (`traceval:`, `tracvals:`) is indistinguishable from another tool's
  section and still yields defaults. The two most likely shapes are covered — keys left at the top
  level, and a wrapper differing only in case — but a wrapper misspelled in any other way is not,
  and cannot be without a registry of known tool names, which is exactly the coupling this decision
  avoids.
...
`test/unit/config.test.ts` pins the behavior end to end against real files in `.tmp/`: the section is
read, sibling tool sections are ignored, an absent file and an other-tools-only file both yield
defaults, and the un-nested config, the orphaned legacy file, a non-mapping root, an unparseable
file, a miscased `Tracevals:` wrapper, an unreadable file, and an invalid value inside the section
each raise a `TracevalsError` — the unreadable case asserting specifically that the legacy file is
not blamed for it. The section-level
validation tests that predate this ADR still exercise `parseConfig()` directly and are unchanged.

## Pros and Cons of the Options

### A single `moose.config.yaml`, each tool reading its own top-level key

- Good, because one file is the whole story: no precedence rules, no merge semantics, no second
...
- Bad, because two sources for one value need precedence rules, and every "why is this value not
...
- Bad, because anchors do not cross file boundaries, so it only helps within one file — which is the
  option above, with extra syntax.
