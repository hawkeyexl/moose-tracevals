...
- [docs/src/content/docs/reference/api.mdx](../docs/src/content/docs/reference/api.mdx) listed
  ...
  `package.json` had never exported it — importing the specifier throws
...
- [docs/content_strategy/information_architecture/ia-gap-analysis.md](../docs/content_strategy/information_architecture/ia-gap-analysis.md)
  ...
  of truth for the `reference/evals-schema` page, sending the next writer who opened the strategy to
...
anchors — seven sections, every one of them about something shaped like a link. Doc Detective runs
what is written as a command. A path or a package specifier quoted inside backticks was invisible to
both, and both errors survived the 01010 change, its review, and later edits to the same files.
...
[test/unit/schema.test.ts](../test/unit/schema.test.ts) is a *behavior* ladder — 13 accepts and 9
rejects. A reworded `description`, a reordered key, an added `$comment`, or a tightened `pattern` no
case exercises all pass it untouched. The invariant was written down twice and enforced nowhere.
...
- **docmeta does not ship the schema.** Its `exports` is `"."` plus `"./package.json"` and its
  `files` is `["dist"]`, so there is no upstream copy on disk to diff against at test time — at any
  version, in any install.
...
Chosen: **a new section 8 in [scripts/check-content-strategy.mjs](../scripts/check-content-strategy.mjs)
that resolves every repo path and package subpath quoted in the docs, and a SHA-256 pin on the
vendored schema in [test/unit/schema.test.ts](../test/unit/schema.test.ts).**
...
identifiers — flags, config keys, grader names, type names — and only a span that begins at a real
repository root directory is treated as a claim about the filesystem.
...
- A span containing `*`, `<`, `>`, `{`, or `}` is a **glob or a placeholder** — a shape, not a file —
  and is skipped.
- `PATH_EXAMPLES` allowlists paths that look like citations but are deliberately illustrative. It
  ...
  [reference/graders.mdx](../docs/src/content/docs/reference/graders.mdx), where a real repository
  path would imply the rule was about that particular file. **Every entry in that set is a place the
  gate is blind**, which is why it is kept to the minimum and why the source says so above the
  declaration.
...
including the reverse drift, where an export is dropped while a page still advertises it.
...
accident and that reads in a diff as *the vendored vocabulary changed* — precisely the event ADR
01010 said must stay visible, and the one the `-proposal.1` prerelease suffix exists to advertise.
...
is the available substitute, and the alternatives that would give a real comparison each cost more
...
- Good, because it reads **source, not rendered HTML**, so it covers `docs/content_strategy/`, which
  is internal and never built into the site — where one of the two findings lived.
- Good, because the pin makes an invariant that existed only in prose fail loudly, and turns a
...
- Bad, because coverage stops at the docs tree. `README.md`, `CLAUDE.md`, and the ADRs themselves
  ...
  fix — but `README.md` and `CLAUDE.md` are simply uncovered.
...
- Bad, because the SHA pin goes red on every legitimate re-sync and someone must update it by hand.
  Accepted: the cost is one line, and the red is the notification.
...
- `npm run docs:check-strategy` — section 8 of
  [scripts/check-content-strategy.mjs](../scripts/check-content-strategy.mjs), run on every PR by
  [docs.yml](../.github/workflows/docs.yml), which gates the Pages deploy. Verified **red before the
  fixes** — reporting `moose-tracevals/schemas/artifact-evals-0.1.json` as undeclared in `exports`
  from `reference/api.mdx`, and both deleted `schemas/artifact-evals-0.*.json` paths from
  `ia-gap-analysis.md` — and green after.
...
- Bad, because a repo path in prose is not a link — there is nothing to crawl. The `api.mdx` finding
  was a literal string in a table cell.
...
- Bad, because it reintroduces the sibling-checkout tax that moving to `@hawkeyexl/inference`
  ...
  wiring again.
- Bad, because a submodule is itself pinned to one revision — the same "matches what we verified"
  property as a digest, at far higher cost.
