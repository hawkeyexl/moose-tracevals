...
Every **behavior change** in tracevals ships with an ADR here. The ADR records the intended behavior and the reasoning — write it before or alongside the code, so it is the reviewable source of truth rather than an afterthought. The full rule lives in [CLAUDE.md](../CLAUDE.md#architecture-decision-records-required).
...
| ADR | Title | Status |
|---|---|---|
| [01000](01000-rebuild-as-trace-adherence-evaluator-on-docevals-and-docmeta.md) | Rebuild tracevals as a trace-adherence evaluator on docevals and docmeta | accepted |
...
| [01008](01008-rename-the-project-to-tracevals.md) | Rename the project to `tracevals` and publish it unscoped | accepted |
