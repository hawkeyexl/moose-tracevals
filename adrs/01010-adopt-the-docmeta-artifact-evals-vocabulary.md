...
This repo published its own eval contract as `schemas/artifact-evals-0.2.json`, under the rule —
inherited from docevals — that *schemas are published by the tool that owns them*, with an explicit
"never register this as a docmeta built-in" clause in [CLAUDE.md](../CLAUDE.md).
...
publishes nine metadata vocabularies, three of which were derived by reworking the in-progress
contracts of three tools in this family — docevals, dockg, and this one. The new line is:
**docmeta publishes common metadata vocabularies; tools implement behavior — graders, graphs,
runtimes — against them.** `docmeta:artifact-evals:1.0.0-proposal.1` *is* our `artifact-evals-0.2`,
reworked into the family shape, and docmeta's own ledger names the debt this repo owes back: a
superseding ADR, plus `extract`/`write`/`fill` reading the new spellings.

The proposal is blunt about timing: the reversal is *"cheap now and only now: docevals and
moose-tracevals have never shipped, so every break below is loud and free."* Confirmed here — zero
git tags, `RELEASE_ENABLED` unset, nothing on npm. `0.1` and `0.2` have no pinned consumers, and
never had any.
...
- The 0.2 shape had real defects the rework fixes: an optional position-derived `name` that
  orphaned history joins, an `evals: []` hole that read as "eval-covered", and a closed grader
  enum that forced a schema version for every new grader.
...
Chosen option: **adopt the docmeta vocabulary now, vendored**, because the shape is already the
better contract and the migration cost only rises from here.
...
unregistered drafts — so no registry work is needed until the review concludes.
...
- *"`schemas/artifact-evals-*.json` are published artifacts — keep each `$id` a resolvable URL."*
  Retired. The `$id` is now docmeta's vendor-prefixed identifier, which is not a URL and should not
...
- **Prefix reservation as a loud-typo guard.** `metadata` is the host tool's extension bag and must
  ...
  run time and rejects any unrecognized `eval*` member, restoring the closed block's property that
  a misspelling is an error rather than a key that quietly does nothing.
...
- Bad, because every existing artifact in the wild breaks. Accepted: there are none — and the 0.2
  spellings fail loudly with a JSON Pointer and a line number, never silently.
...
- [test/unit/schema.test.ts](../test/unit/schema.test.ts) is a case-for-case port of docmeta's own
  verification ladder (`docs/proposals/0023/ladders/artifact-evals-examples.cjs`) — 13 accepts and
  9 rejects, including the migration negatives (the `criteria` envelope, the old `name` key, and
  the command-family guard rails). Drift between our copy and docmeta's draft fails here.
...
- Bad, because the review may take a while, and the "breaks are free" window closes at first
  publish — which is a one-time setup step away.
