...
`.claude/commands/*.md` are frontmattered markdown whose body Claude Code injects as a prompt — the
same shape as a skill, and exactly what this tool exists to check. They were invisible: `ArtifactType`
was `"skill" | "agent" | "project-rules"`, so a command declaring `metadata.evals` was never
...
`/model` and `/code-review` read as skills that were never on the roster, which is a confident
...
- **The transcript records the name and nothing else.** Every injection is the three tags
  `<command-name>`, `<command-message>`, `<command-args>` — never the command's body, never its
...
- **A command's frontmatter is consumed by the harness.** `description`, `argument-hint`,
...
- No verdict on the fixture corpus may move except the ones that were wrong.
  ...
  so nothing here may add an eval field — and nothing here needs to.
...
- `fill` writes by default (ADR 01005), so any convention it recognizes must be narrow enough that
  it cannot edit prose that merely lives in a directory called `commands`.
...
- The injection's own span, up to the next injection — the skill rule
- The whole session
...
- No — `SkillInvocation.via` already carries the distinction
- Yes, a parallel list on `Trace`

## Decision Outcome

Chosen option: **add `"slash-command"` to `ArtifactType`; let resolution decide which of three
things a `/name` is by looking for the file; window it exactly as a skill; make it writable by
`fill`; report an unresolvable one as a built-in without a warning; and add no field to `Trace`.**
...
| 2 | the same two directories, searched recursively | a `slash-command` artifact — an organizing subdirectory does not rename the command |
...
first half, and the row keeps its full `tried` list, so a command that genuinely *is* missing on this
...
about evidence that was never collected — and making that claim anyway *was* the defect ADR 01016
...
`Skill` tool call can close a command's window but never opens one: the tool is not the slash-command
mechanism, and letting it open the window would grade a command against turns nobody ran it for.
...
acting are teaching to the test" — and, decisively, because a rules file's **frontmatter is injected
verbatim**. A skill's is not: Claude Code parses it and surfaces only `name` and `description`, which
is why skills have always been writable.
...
than shown. The harness consumes the frontmatter; only the body becomes the prompt. So a
`metadata.evals` block in a command file cannot reach the session under test, and `fill` treats these
files exactly as it treats skills — appending, with an `eval-provenance` entry naming the model and
its confidence.
...
decision; and `skill-invoked` is excluded for the reason it is excluded from skills — an artifact
...
prefix relative to the scanned anchor. That is the same two-step `isRecognizedAgent` uses and for the
...
list would duplicate the same records, force every synthetic `Trace` literal in the suite to grow a
field for the fourth time in this stack (ADR 01013, 01014, 01016), and buy nothing that a one-field
filter does not. The type's doc comment now says what `via` means for consumers, which is the change
that was actually missing.

`PROMPT_VERSION` and `FILL_PROMPT_VERSION` are both unchanged. The judge prompt is untouched, and the
fill prompt is byte-identical for every pre-existing artifact type — a slash command is a new value of
`artifactType`, which is already a cache-key component, so its entries are new keys rather than stale
ones.
...
- Good, because it is retroactive. Every session already on disk gets the better answer, and the real
  session that motivated ADR 01016's limitation now reports `/model` and `/code-review` as
  `slash-command` rows with **zero warnings**, where it previously produced two "no SKILL.md was
  found" warnings and two `not-offered` skill rows.
...
- Bad, because "built-in" is an inference from absence, so a *project* command that exists on another
  ...
  used `/model`, while this one is imprecise only on a session run against a checkout that is missing
  a command file.
...
- Bad, because a command invoked only inside a subagent branch is windowed to that branch's chain,
  ...
  tool calls without its turns — inherited from ADR 01015 rather than introduced here.
- Neutral, because output styles were considered alongside commands and left out: they are not
  ...
  stay out for the reason the roadmap gives — JSON configuration, not instruction prose.

### Confirmation

`test/unit/artifacts.test.ts` pins all four resolution branches against the committed corpus: a
project command from `.claude/commands`, a plugin command from the store, a `/name` that resolves to
a `SKILL.md` staying a **skill** row, and a built-in producing a `slash-command` row with a note, no
skill row, and no warning.

`test/unit/graders/window.test.ts` pins the window in both directions — closed by the next injection,
and *not* opened by a `Skill` tool call of the same name, which is the one way a command could be
graded against turns nobody ran it for. `test/unit/artifact-availability.test.ts` pins that no
...
and `test/fixtures/traces/claude-session.jsonl` invokes it — after two `Bash` calls that both fall
outside its window, so `no-shell-during-release` passing is the windowing assertion rather than an
accident. The same trace invokes `/model`, which resolves to nothing, and
...
and absent warning, both windowed verdicts, and — on the `fill` steps — that the command is
...
- Good, because it is deterministic, recorded (`tried`), and degrades to a coverage note rather than
  a crash — the contract ADR 01003 already set for every other reference.
- Good, because it handles the case nobody would have hard-coded: a skill invoked by typing its slash
  form, which is how Claude Code surfaces skills and therefore common.
...
- Bad, because `inlineEval` is `additionalProperties: false` in a vendored copy of docmeta's schema,
  so the field does not exist and could not ship here without an upstream proposal — and the type
...
- Bad, because it would misread ADR 01005's actual reason. The hazard there is frontmatter injected
  *verbatim* into the model's context; a command's frontmatter is consumed by the harness, so the
  rubric never reaches the session. Refusing to write would leave the one artifact type whose evals
  people most want bulk-authored — commands are short and procedural — permanently manual, for a risk
  that the transcript shows does not exist.
...
- Bad, because it duplicates records already in `skillInvocations`, and the window rule needs the two
  interleaved anyway to compute boundaries — so the list would have to be re-merged at every use.
