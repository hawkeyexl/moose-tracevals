...
`command` grader and chose **default-on**. That reasoning stands and is not revisited here. But it
considered only those two shapes, so the risk it names in its own consequences —

> `moose-tracevals run` over an untrusted trace can execute that project's code.

— has no lever for the person taking it. There is nothing to type, and nothing to put in a config,
...
- The risk is real, documented, and accepted — but a documented risk with no mitigation available to
...
- A check that did not run has not been satisfied. Whatever the disabled state reports, it cannot be
  `pass`.
- The knob belongs to the *evaluating* side — the person running the tool — not to the artifact
  declaring the eval. An artifact cannot exempt itself from a lever pulled against it.
- Two audiences want it for different reasons: someone triaging an unfamiliar project's trace, and a
  CI job that wants to prove nothing in an evaluated repo can execute.
...
- Nothing: keep documenting the risk and pointing at `--deterministic-only`

## Decision Outcome

Chosen option: **`--no-commands` plus `tracevals.graders.command.enabled`, reporting `skipped` with
a stated reason.** This **amends ADR 01011 rather than superseding it** — 01011's decision (run by
default) is unchanged, and this adds the third option it never considered.
...
`skipped`, not `pass`, and not `error`. `pass` would be a lie about a check that never ran — the
...
- Good, because the person exposed to the execution surface finally has the lever, in both the
  place a human types and the place a repo commits.
...
- Neutral, because the opt-out still lives in the evaluating repo's config — 01011's objection to
  gating "the wrong side" applies to an opt-*in* that silently breaks declared evals, not to an
  opt-out someone deliberately reaches for.
...
- `test/unit/engine.test.ts` pins the three properties that matter: commands run by default, a
  ...
  same outcomes for every other eval, same exit code.
...
- Bad, because the flag would then fail every run that used it, so nobody could adopt it in CI —
  which is the main place it is wanted.
...
- Bad, because it needs a matching, path-normalizing, cross-platform comparison to be worth
  anything, which is a lot of surface for a knob nobody has asked for yet. The on/off switch does
