 ...
 *   5. Every `/moose-tracevals/...` link in a published page points at a page that
 *      exists.
...
// (`](/moose-tracevals/…)`) and component props (`href="/moose-tracevals/…"`) are the
...
const SITE_LINK = /(?:href=|]\(|^\s*link:\s*)"?(\/moose-tracevals\/[^")\s]*)"?/gm;
...
for (const f of sources) {
  for (const m of readFileSync(f, "utf8").matchAll(/(\/moose-tracevals\/[^"')\s]*?)#([a-z0-9-]+)/g)) {
    anchors++;
